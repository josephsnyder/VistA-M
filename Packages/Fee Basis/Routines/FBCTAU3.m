...
DE S DIE="^FBAAA(D0,1,",DIC=DIE,DP=161.01,DL=2,DIEL=1,DU="" K DG,DE,DB Q:$O(^FBAAA(D0,1,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,19) S:%]"" DE(1)=% S %=$P(%Z,U,20) S:%]"" DE(2)=%
 I $D(^("C")) S %Z=^("C") S %=$P(%Z,U,1) S:%]"" DE(3)=%,DE(6)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
1 S DW="0;19",DV="FXO",DU="",DLB="ACCIDENT RELATED (Y/N)",DIFLD=.096
 S DQ(1,2)="S Y(0)=Y D OUTYN^FBAAUTL3"
 S DE(DW)="C1^FBCTAU3"
 G RE
...
X1 I $D(X) D YN^FBAAUTL3
 ...
 Q
 ;
2 D:$D(DG)>9 F^DIE17,DE S DQ=2,DW="0;20",DV="RFXO",DU="",DLB="POTENTIAL COST RECOVERY CASE",DIFLD=.097
 S DQ(2,2)="S Y(0)=Y D OUTYN^FBAAUTL3"
 S DE(DW)="C2^FBCTAU3"
 S X="NO"
 S Y=X
 G Y
...
C2F1 Q
X2 I $D(X) D YN^FBAAUTL3
 I $D(X),X'?.ANP K X
 Q
 ;
3 D:$D(DG)>9 F^DIE17,DE S DQ=3,DW="C;1",DV="FX",DU="",DLB="PRINT AUTHORIZATION (Y/N)",DIFLD=1
 S DE(DW)="C3^FBCTAU3"
 S X="YES"
 S Y=X
 G Y
