...
DE S DIE="^FBAAA(D0,1,",DIC=DIE,DP=161.01,DL=2,DIEL=1,DU="" K DG,DE,DB Q:$O(^FBAAA(D0,1,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,4) S:%]"" DE(11)=% S %=$P(%Z,U,8) S:%]"" DE(1)=% S %=$P(%Z,U,14) S:%]"" DE(8)=% S %=$P(%Z,U,19) S:%]"" DE(14)=% S %=$P(%Z,U,20) S:%]"" DE(15)=%
...
1 S DW="0;8",DV="F",DU="",DLB="DX LINE 1",DIFLD=.08
 G RE
X1 K:$L(X)>60!($L(X)<3) X
 I $D(X),X'?.ANP K X
 Q
...
7 S D=0 K DE(1) ;.021
 ...
 ;
8 S DW="0;14",DV="S",DU="",DLB="TYPE OF CARE",DIFLD=2
 S DU="1:C&P;2:OPT NSC;3:OPT SC;"
 G RE
X8 Q
9 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=9 D X9 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X9 S:'$D(FBAAASKV) FBAAASKV="N"
 Q
10 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=10 D X10 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X10 S:FBAAASKV'="y" Y=100
 Q
11 S DW="0;4",DV="P161.2",DU="",DLB="VENDOR",DIFLD=.04
 S DE(DW)="C11^FBCTAU2"
 S DU="FBAAV("
 G RE
